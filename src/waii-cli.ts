...
const help = (cmd = '', scmd = '') => {
    if (cmd && scmd) {
        const scmdTree = callTree[cmd as keyof typeof callTree];
        const doc = scmdTree[scmd as keyof typeof scmdTree]['doc'];
        if (!doc) {
...
function printCommands(commands: Record<string, unknown>, level = 0) {
    for (const o in commands) {
        process.stdout.write('   '.repeat(level) + o + ':');
        if (level === 0) {
            console.log('');
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            printCommands(commands[o] as any, level + 1);
        } else {
            if (typeof commands[o] === 'object') {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                console.log(' '.repeat(22 - o.length - 4) + ((commands[o] as any)['doc'] ? (commands[o] as any)['doc']['description'] : ''));
            }
        }
    }
    console.log('');
...
const initialize = async () => {
    const configPath = process.env.HOME + CONF_FILE.slice(1);
    ...
    if (!fs.existsSync(configPath)) {
        ...
        const yamlStr = YAML.stringify(placeholderConfig);
...
const main = async () => {
    try {
        ...
        const params = await parseInput(process.argv);
        if (params.cmd === 'help') {
            help(params.scmd, params.vals[0]);
        } else {
            const scmdTree = callTree[params.cmd as keyof typeof callTree];
            if (!scmdTree) {
                throw new ArgumentError('Unknown command ' + params.cmd);
            }

            const callObj = scmdTree[params.scmd as keyof typeof scmdTree];
            if (!callObj) {
                throw new ArgumentError('Unknown subcommand ' + params.scmd);
            }

            const fn: (arg: CmdParams) => void = callObj['fn'];
            if (!fn) {
